...
const RANDOM_INTEGERS = [];

const getPhoto = () => {
  let PHOTO_NUMBER;

  do {PHOTO_NUMBER = getRandomInteger(0, PHOTOS.length-1);}
  while (RANDOM_INTEGERS.includes(PHOTO_NUMBER));

  RANDOM_INTEGERS.push(PHOTO_NUMBER);
  return PHOTOS[PHOTO_NUMBER];
};
...
const getRandomId = () => {
  ...
  do {USERS_ID = getRandomInteger(0, 1000);}
  while (ID_ARRAY.includes(USERS_ID));

  ID_ARRAY.push(USERS_ID);
  return USERS_ID;
};
